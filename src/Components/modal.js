...
export default function ToggleModal() {
  ...
  const content=useMemo(()=>{if(modalTitle==="Specilists Needed"){
    ...
    <>
      ...
      <MDBModal show={show} >
        <MDBModalDialog>
          <MDBModalContent>
            <MDBModalHeader>
{   modalTitle==="Select Date" ?  <MDBModalTitle style={{color:"brown"}}><BsCalendarDate/><span style={{marginLeft:"10px"}}>{modalTitle}</span></MDBModalTitle>
             : <MDBModalTitle style={{color:"red"}}><ImNotification color={'red'} /><span style={{marginLeft:"10px"}}>{modalTitle}</span></MDBModalTitle>}
            </MDBModalHeader>
          { modalTitle!=="Select Date" ? <MDBModalBody style={{color:"Black"}}><span style={{color:"black"}}>{content}</span>  </MDBModalBody>:
          <MDBModalBody> 
           <span >{modalTitle==="Select Date" && <MDBInput type="date" onChange={(e)=>setSelectedDay(e.target.value)}/>}</span>
          </MDBModalBody>
}

            <MDBModalFooter>
            
              <MDBBtn className='cancelBtn' onClick={()=>dispatch(modalShow(""))}>Cancel</MDBBtn>
              {modalTitle==="Select Date" &&<MDBBtn  className="saveBtn" onClick={()=>handleSave()}>
          save
              </MDBBtn>}
            </MDBModalFooter>
          </MDBModalContent>
